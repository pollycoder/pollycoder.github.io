...
```matlab
bounds.phase(p).initialtime.lower = t0_lower;						% Initial time - lower bound
bounds.phase(p).initialtime.upper = t0_upper;						% Initial time - upper bound
bounds.phase(p).finaltime.lower = tf_lower;							% Final time - lower bound
bounds.phase(p).finaltime.upper = tf_upper;							% Final time - upper bound
bounds.phase(p).durations.lower = dt_lower;							% tf-t0 - lower bound (optional)
bounds.phase(p).durations.upper = dt_upper;							% tf-t0 - upper bound (optional)
```
2. State Bounds:  $size=1\times n_y^{(p)}$

```matlab
bounds.phase(p).initialstate.lower = state0_lower;			% Initial state - lower bound
bounds.phase(p).initialstate.upper = state0_upper;			% Initial state - upper bound
bounds.phase(p).finalstate.lower = statef_lower;				% Final state - lower bound
bounds.phase(p).finalstate.upper = statef_upper;				% Final state - upper bound
bounds.phase(p).state.lower = state_lower;							% State - lower bound
bounds.phase(p).state.upper = state_upper;							% State - upper bound
```
3. Control Bounds: $size=1\times n_u^{(p)}$

```matlab
bounds.phase(p).control.lower = u_lower;								% Control - lower bound
bounds.phase(p).control.upper = u_upper;								% Control - upper bound
```
4. Path Constraint Bounds: $size=1\times n_c^{(p)}$

```matlab
bounds.phase(p).path.lower = u_lower;										% Path constraint - lower bound
bounds.phase(p).path.upper = u_upper;										% Path constraint - upper bound
```
5. Integral Bounds: $size=1\times n_d^{(p)}$

```matlab
bounds.phase(p).integral.lower = u_lower;								% Integral - lower bound
bounds.phase(p).integral.upper = u_upper;								% Integral - upper bound
```
6. Parameter Bounds (if defined): $size=1\times n_s $

```matlab
bounds.phase(p).parameter.lower = u_lower;							% Parameters - lower bound
bounds.phase(p).parameter.upper = u_upper;							% Parameters - upper bound
```
7. Eventgroup Bounds (if defined): $size=1\times n_b$

```matlab
bounds.phase(p).eventgroup(g).lower = u_lower;					% Eventgroup - lower bound
bounds.phase(p).eventgroup(g).upper = u_upper;					% Eventgroup - upper bound
```
...
1. Time Guess: $size=M^{(p)}\times1$

```matlab
guess.phase(p).time = t_guess;
```

2. State Guess: $size=M^{(p)}\times n_y^{(p)}$

```matlab
guess.phase(p).state = state_guess;
```

3. Control Guess: $size=M^{(p)}\times n_u^{(p)}$

```matlab
guess.phase(p).control = control_guess;
```

4. Integral Guess: $size=1\times n_d$

```matlab
guess.integral = integral_guess;
```

5. Parameter Guess: $size=1\times n_s$
